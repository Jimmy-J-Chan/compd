...
def get_client_screen_data(key=''):
    ...
    dim_screen['screen_iwidth'] = streamlit_js_eval(js_expressions='window.innerWidth', key=f"iw_{key}")
    dim_screen['screen_iheight'] = streamlit_js_eval(js_expressions='window.innerHeight', key=f"ih_{key}")
    return dim_screen
